const {ATTRIBUTES_KEY} = require("../fileProcessor");
...
describe('applyIndicationMapping', () => {
    ...
    it('should map string values based on the provided mapping', () => {
        ...
        expect(new Mapper(mapping, ATTRIBUTES_KEY).applyMapping(input)).toEqual(expected);
    });
    ...
    it('should retain the value if the key is ATTRIBUTES_KEY', () => {
        const input = { [ATTRIBUTES_KEY]: { attr1: "A", attr2: "value2" }, key1: "A" };
        const expected = { [ATTRIBUTES_KEY]: { attr1: "A", attr2: "value2" }, key1: "Mapped A" };
        expect(new Mapper(mapping, ATTRIBUTES_KEY).applyMapping(input)).toEqual(expected);
    });
});
